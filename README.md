...
| Version | Highlights |
|---|---|
| [v1.4.1](https://github.com/GigaShalgham/wifi-data-guard/releases/tag/v1.4.1) | **Deep-scan fixes (spec-012) — the v1.2 "won't unlock tomorrow" bug is dead.** The watchdog now records **which period a lock engaged in**; if the app restarts after midnight (reboot, battery killer, app update) and the quota has reset, a limit lock releases itself — no more "turn Enforce limit off and on again". Safety unchanged: parent/offline/clock-tamper locks still survive rollovers, and a rolled-back clock can never fake a reset (5 new unit tests, incl. a migration that unsticks a pre-v1.4.1 latch on first launch). Also: the history chart's **Farsi weekday labels now match the real weekdays** (every glyph was off before), and the device report now includes the coarse latch reason |
| dashboard spec-012 (no APK needed) | **Honest full unlock + reason badges (dg-v9).** A locked card now shows **why** it's locked ("LOCKED · limit / parent / offline / clock", EN+FA) and during a timed unlock the badge says **grace**, not LOCKED (matches the app's FREE TIME hero — Art. VIII). Pressing **∞ Full unlock** on a limit-locked phone now warns the truth ("it will get a 15-minute window instead"), the pending chip confirms when the window lands, and the toast says "✓ N-min window granted — limit lock kept". Old apps (no latch_reason) keep today's behavior; revoking a device now also drops its undeliverable commands (SW cache dg-v9 — one reload at most) |
