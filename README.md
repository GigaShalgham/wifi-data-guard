...
| Version | Highlights |
|---|---|
| [v1.4.2](https://github.com/GigaShalgham/wifi-data-guard/releases/tag/v1.4.2) | **Deep-scan fixes (spec-012 amendment) — honest full unlock + the stuck-phone unstick.** A phone updating from a pre-v1.4.1 build with a stale limit latch now **unblocks itself on first launch** (the boundary processes once; genuinely fresh usage releases it, same-day over-limit usage re-locks on the very same tick — never a free unlock — and unreadable stats keep the lock, Art. II). The device report now carries the **coarse latch reason**, so the dashboard can be honest (see the dg-v9 row below). Also: the history chart's **Farsi weekday labels now match the real weekdays** (every glyph was off before; the test now pins real dates). Release gate grew to 29 unit tests incl. 7 cold-start rollover cases |
| dashboard spec-012 amendment (no APK needed) | **Honest full unlock + reason badges (dg-v9).** A locked card now shows **why** it's locked ("LOCKED · limit / parent / offline / clock", EN+FA) and during a timed unlock the badge says **grace**, not LOCKED (matches the app's FREE TIME hero — Art. VIII). Pressing **∞ Full unlock** on a limit-locked phone now warns the truth ("it will get a 15-minute window instead"), the pending chip confirms when the window lands, and the toast says "✓ N-min window granted — limit lock kept". Old apps (no latch_reason) keep today's behavior; revoking a device now also drops its undeliverable commands; dead pair-code counter removed (SW cache dg-v9 — one reload at most) |
| [v1.4.1](https://github.com/GigaShalgham/wifi-data-guard/releases/tag/v1.4.1) | **Hotfix (spec-012): "locked until tomorrow" now survives restarts.** The daily-limit latch released only if the guard process was alive across midnight — after an overnight reboot, dead battery, OEM task-killer or app update, the phone stayed locked the next day (and every day after) until the parent toggled the enforce switch off/on. The period rollover detector is now **persisted** (`period_start`), so a restart after the boundary processes it identically to a live midnight crossing. Fail-closed guard added: if usage stats are unreadable at the boundary, the limit latch is kept and retried each tick (a restart + revoked stats can never become a free unlock); security latches (remote lock / fail-closed / clock tamper) still survive the boundary. New Robolectric release gate: `RolloverRestartTest` cold-starts the real watchdog in the stuck state (6 cases). **App update required** — Worker/dashboard untouched (dg-v8 stays) |
| [v1.4.0](https://github.com/GigaShalgham/wifi-data-guard/releases/tag/v1.4.0) | **UX revolution (spec-010) — app restructure + real charts.** The app is now a three-tab experience: **Status** (animated gradient usage ring with count-up percent, state pill, unlock button), **Usage** (count-up big number, stats grid — left/limit/period-end/battery — and a **7-day usage history chart** from the cloud, honest "no data" markers, stale-cache-first), **Parent** (everything gated behind one PIN per session: settings with **unlock-duration chips replacing the spinner**, all tools). Bottom navigation with a red badge dot while blocked, aurora background, tab cross-fades — all reduce-motion safe. New device-authenticated `/api/child/history` API on the Worker; history is purely cosmetic (never latches) |
| dashboard spec-011 (no APK needed) | **One Unlock button (merged).** The two side-by-side buttons on a locked device card ("Unlock" + "⏱ Timed unlock") are now a single **Unlock** button with an ordered flow: tap it and the picker ladder offers quick chips (15/30/60/120) → custom minutes (1–480) → a distinct amber **∞ Full unlock — until you lock it again** row (same safety warning as before, keyboard-operable). **Press-and-hold the Unlock button** to instantly repeat your last duration — a "⚡ Quick unlock: N min" toast confirms (EN + FA with Persian digits; the hold gesture never triggers the unbounded full unlock). Zero changes to commands or payloads — every app ≥ v1.3.3 works as-is (SW cache dg-v8 — one reload at most) |
| dashboard spec-010 (no APK needed) | **The Unlock window (min) settings field is GONE** — replaced by a point-of-use duration picker: tap "⏱ Timed unlock" and pick 15/30/60/120 min or a custom 1–480 right there; your last choice is remembered per device (EN + FA with Persian digits). Also fixes the battery line mojibake (`ὐb` → 🔋) and adds the `/api/child/history` endpoint (SW cache dg-v7 — one reload at most to see it) |
| [v1.3.6](https://github.com/GigaShalgham/wifi-data-guard/releases/tag/v1.3.6) | **Hotfix (spec-009): v1.3.5 crashed at launch on every device** — a Kotlin type-inference bug in the new entrance animation allocated the view list as `Button[]` and crashed with `ArrayStoreException` during cold start (found and fixed with the project's first launch smoke test, now permanent: every release must pass a Robolectric cold-start test in two states — fresh install and the armed kid's phone); also hardened the unlock-duration spinner against callback re-entrancy. Update any crashing v1.3.5 install — settings/PIN/pairing are preserved |
| dashboard spec-008 (no APK needed) | Settings bugfixes: an open Settings panel now keeps your edits (a per-device draft survives the 10 s heartbeat, post-command refreshes and stale responses — no more reverting 200 back to 500), the save button can't double-fire, a failed save shows an honest red error, and the save is confirmed by the device ("Settings → device…" chip → "✓ Settings applied" toast). The **Unlock window (min)** setting is now real: the timed-unlock button, its label, its confirm dialog and its toast all use the configured window (1–480 min, 0/invalid → 15) instead of a hardcoded 15 |
