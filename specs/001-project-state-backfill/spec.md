...
- **Latest release**: v1.3.6 (versionCode 10) — launch-crash hotfix for v1.3.5 (spec-009); tags exist for v1.0–v1.3, v1.3-test1/2, v1.3.2, v1.3.2-test3, v1.3.3, v1.3.3-test4, v1.3.4, v1.3.4-test5, v1.3.5, v1.3.5-test6, v1.3.6, v1.3.6-test7
...
| # | Spec | Why | Source |
|---|------|-----|--------|
| A | `010-cloud-security-hardening` | Remove public `/demo`; harden/remove the bootstrap on-screen sign-in link (leaked bootstrap email = takeover chain); commit-email hygiene (noreply) | Task 16 audit + user decision pending on email path |
| B | `011-play-store-hardening` | allowBackup=false audit, exported components, targetSdk policy, privacy declaration | Task 8 notes |
| C | `012-e2e-verification-v1.3.6` | Device re-tests (AFTER installing v1.3.6): app opens + glass UI + toasts/chime (spec-007/009), instant commands + ⚡ chip (spec-005), full unlock (spec-003), refresh-mid-command (spec-004), safe-unpair carry-over (spec-002), settings save + unlock-window from the panel (spec-008) | specs 002–009 |
