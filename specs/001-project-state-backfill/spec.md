...
- **Latest release**: v1.4.0 (versionCode 11) — UX revolution: three-tab app, usage ring, 7-day history chart, chips, Parent PIN gate (spec-010); tags exist for v1.0–v1.3, v1.3-test1/2, v1.3.2, v1.3.2-test3, v1.3.3, v1.3.3-test4, v1.3.4, v1.3.4-test5, v1.3.5, v1.3.5-test6, v1.3.6, v1.3.6-test7, v1.4.0, v1.4.0-test8
...
| ✓ | ~~`012-restart-safe-rollover`~~ DONE (app v1.4.1 + worker dg-v9, 2026-09-18) | Deep-scan fixes: restart-safe rollover (v1.2 "won't unlock tomorrow" root-caused and fixed — latch_period_start + RolloverPolicy reconcile), honest full unlock on limit latches (report `latch_reason` + truthful warning/toast + reason badges + grace precedence), FA weekday fix, revoked-device command cleanup | owner directive ("deep scan of the full system and debug anything we didn't see") + v1.2 bug report |
...
| C | `015-e2e-verification-v1.4.1` | Device re-tests (AFTER installing v1.4.1 AND re-pairing — the phone's pairing was revoked Sep 15): app opens with tabs + ring + history chart, ONE merged Unlock button (tap → ladder incl. ∞ full row; hold → quick unlock), reason-aware LOCKED badge + honest full-unlock warning (spec-012), **overnight auto-unlock after a restart past midnight (spec-012 headline fix — the v1.2 report)**, settings has no unlock field, instant commands + ⚡ chip, full unlock (spec-003), refresh-mid-command (spec-004), safe-unpair carry-over (spec-002), settings draft save (spec-008), launch stability (spec-009) | specs 002–012 |
