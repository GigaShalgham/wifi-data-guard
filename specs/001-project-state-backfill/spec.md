...
- **Latest release**: v1.3.1 (versionCode 5, commit `d1bdadd`); tags exist for v1.0–v1.3, v1.3-test1, v1.3-test2
...
| # | Spec | Why | Source |
|---|------|-----|--------|
| A | `002-remove-demo-endpoint` | Kill the public `/demo` simulator before real families use the Worker | Task 8/11 notes |
| B | `003-play-store-hardening` | allowBackup=false audit, exported components, targetSdk policy, privacy declaration | Task 8 notes |
| C | `004-e2e-verification-v1.3.1` | Formalize the user's device re-tests of v1.3-test2 (TC-10/11/09, soft-fallback alert) into a results record | Task 11 |
| D | `005-v1.3.1-tag-release` (tiny) | v1.3.1 shipped to main; verify it has a proper tag/release page, publish if missing | Task 11 |
