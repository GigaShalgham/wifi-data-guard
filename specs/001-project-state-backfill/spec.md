...
| # | Spec | Why | Source |
|---|------|-----|--------|
| A | `009-cloud-security-hardening` | Remove public `/demo`; harden/remove the bootstrap on-screen sign-in link (leaked bootstrap email = takeover chain); commit-email hygiene (noreply) | Task 16 audit + user decision pending on email path |
| B | `010-play-store-hardening` | allowBackup=false audit, exported components, targetSdk policy, privacy declaration | Task 8 notes |
| C | `011-e2e-verification-v1.3.5` | Device re-tests (AFTER installing v1.3.5-test6): instant commands + ⚡ chip (spec-005), glass UI + toasts on BOTH app and dashboard (spec-006/007), full unlock (spec-003), refresh-mid-command (spec-004), safe-unpair carry-over (spec-002), settings save + unlock-window from the panel (spec-008) | specs 002–008 |
