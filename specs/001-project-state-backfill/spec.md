...
| ✓ | ~~`012-restart-safe-rollover`~~ DONE (app v1.4.1, 2026-09-17) | Persisted period rollover: a limit latch releases after an overnight restart (reboot/battery/OEM killer/update); fail-closed when stats unreadable | owner bug report ("won't unlock over time or tomorrow… enforce off and on again") |
...
| C | `015-e2e-verification-v1.4.1` | Device re-tests (AFTER installing v1.4.1): app opens with tabs + ring + history chart, ONE merged Unlock button on the dashboard (tap → ladder incl. ∞ full row; hold → quick unlock, spec-011), settings has no unlock field, instant commands + ⚡ chip (spec-005), full unlock (spec-003), refresh-mid-command (spec-004), safe-unpair carry-over (spec-002), settings draft save (spec-008), launch stability (spec-009), **overnight-restart unlock (spec-012: reboot the phone while limit-latched in the evening → next morning it must be unlocked without toggling enforce)** | specs 002–012 |
