...
- **Latest release**: v1.4.2 (versionCode 13) — restart-safe rollover + recovery/reporting fixes; tags exist for v1.0–v1.4.2 incl. all QA builds (test1–test10). All 22 release notes were professionalized 2026-09-19 (spec-013: formal third-person bodies, no private-conversation content, no internal-governance headers)
...
| ✓ | ~~`013-release-professionalization`~~ DONE (release + repo metadata, 2026-09-19) | All 22 GitHub releases rewritten formally (third-person bodies, no private-conversation quotes, no internal-governance headers; titles "vX.Y.Z — description"); README demo link + CF badge. Repo description/homepage/20 topics staged in `scripts/spec013_bodies.py` (dev sandbox) — **API application blocked by token scope (fine-grained PAT lacks Administration:write); owner must apply via UI or upgrade the token, then re-run `release_professionalize_spec013.py`** | owner directive (formal public releases + SEO) |
| A | `014-cloud-security-hardening` | Remove public `/demo`; harden/remove the bootstrap on-screen sign-in link (leaked bootstrap email = takeover chain); commit-email hygiene (noreply); optional: server-time-authoritative period (clock-forward unlock hole documented in spec-012 FM2) — **also: if /demo is removed, update the repo homepage + README demo link** | Task 16 audit + user decision pending on email path |
| B | `015-play-store-hardening` | allowBackup=false audit, exported components, targetSdk policy, privacy declaration | Task 8 notes |
| C | `016-e2e-verification-v1.4.2` | Device re-tests (AFTER installing **v1.4.2** AND **re-pairing** — D1 shows the phone's pairing was revoked Sep 15, so remote control is inert until a new code is entered): app opens with tabs + ring + history chart, ONE merged Unlock button (tap → ladder incl. ∞ full row; hold → quick unlock), reason-aware LOCKED badge + honest full-unlock warning (spec-012 amendment), **overnight-restart unlock (reboot the phone while limit-latched in the evening → next morning it must be unlocked without toggling enforce — the v1.2 report)**, settings has no unlock field, instant commands + ⚡ chip (spec-005), full unlock (spec-003), refresh-mid-command (spec-004), safe-unpair carry-over (spec-002), settings draft save (spec-008), launch stability (spec-009) | specs 002–012 |
